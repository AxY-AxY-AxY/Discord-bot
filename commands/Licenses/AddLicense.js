...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator), // Restrict to admin role
    async execute(interaction) {
        let idfrom = interaction.guild ? interaction.guild.id : interaction.user.id;

        let sellerkey = await db.get(`token_${idfrom}`);
        if (sellerkey === null) return interaction.reply({ embeds: [new EmbedBuilder().setDescription(`Your \`SellerKey\` **has not been set!**\nIn order to use this bot, you must run the \`/add-application\` command first.`).setColor(Colors.Red).setTimestamp()], ephemeral: true });
        ...
        if (amount > 20) return interaction.reply({ embeds: [new EmbedBuilder().setTitle('Failure').addFields([{ name: 'Reason:', value: `You cannot add more than twenty keys at a time.` }]).setColor(Colors.Red).setFooter({ text: "KeyAuth Discord Bot" }).setTimestamp()], ephemeral: true });
            ...
            fetch(`https://keyauth.win/api/seller/?sellerkey=${sellerkey}&type=add&expiry=${days}&mask=${license_mask}&level=${level}&amount=${amount}&character=${character}&note=${note}&format=text`)
                ...
                .then(text => {
                    if (!text.includes("message")) {
                        ...
                        interaction.reply({ embeds: [new EmbedBuilder().setTitle(json.message).addFields([{ name: 'Note:', value: `Your seller key is most likely invalid. Change your seller key with \`/add-application\` command.` }]).setColor(Colors.Red).setFooter({ text: "KeyAuth Discord Bot" }).setTimestamp()], ephemeral: true });
                    }
                });
        } else {
            let licenseAdd = await db.get(`licenseAdd_${idfrom}`);
            if (licenseAdd === null) return interaction.reply({ embeds: [new EmbedBuilder().setDescription(`No config saved for adding licenses yet. Please do a command with parameters included then this will work.`).setColor(Colors.Red).setTimestamp()], ephemeral: true });
