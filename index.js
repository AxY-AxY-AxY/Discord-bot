...
client.once('ready', async () => {
    ...
    await setPresence(client, `a`, { type: ActivityType.Competing, status: 'online' });
});
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.member != null)
        if (!interaction.member.roles.cache.find(x => x.name == "Staff")) return interaction.editReply({ embeds: [new EmbedBuilder().setDescription(`You need a role with the name \`Staff\` to execute commands. Please ask an administrator to create a role with this name if not already done and assign it to you.`).setColor(Colors.Red).setTimestamp()], ephemeral: true })
